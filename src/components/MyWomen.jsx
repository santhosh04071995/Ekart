...
import wstyle from './MyWomen.module.css'
...
export default function MyWomen() {
    const[lists,setLists] = useState([]);
    const [smShow, setSmShow] = useState(false);
    useEffect(()=> {
        async function myfunc() {
             let x = await fetch("https://fakestoreapi.com/products/category/women's clothing");
             let y = await x.json();
             console.log(y);
             setLists(y);
         }
         ...
         {/* End of cart popup */}
           ...
           <div className="row ms-3">
                ...
                {lists.map((list)=> (
                    ...
                    <Card.Img className={wstyle.pimage} variant="top" src={list.image} />
                    <Card.Body>
                      <Card.Title className={wstyle.mytitle}>{list.title}</Card.Title>
